...
```
Codebase → Project docs → MCP tools (if available) → Web search → Flag as uncertain
```
...
Overview of how components interact. Use mermaid diagrams when helpful to visualize component relationships.

### 3. Identify Code Reuse

**CRITICAL**: What existing code can we leverage? This saves tokens and reduces errors.

If `/.specs/project/STATE.md` exists, check it before designing. Any component flagged as fragile, carrying tech debt, or having test coverage gaps requires extra care in the design — document how the design mitigates those concerns.
...
- **Keep it visual** — Diagrams save 1000 words (use mermaid syntax)
- **Small components** — If component does 3+ things, split it
- **Check STATE.md** — If it exists, flag fragile areas the design must address
